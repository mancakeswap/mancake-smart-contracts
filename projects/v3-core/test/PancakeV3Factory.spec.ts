...
import { MancakeV3PoolDeployer } from '../typechain-types/contracts/MancakeV3PoolDeployer'
...
describe('PancakeV3Factory', () => {
  ...
  let deployer: MancakeV3PoolDeployer
  let factory: PancakeV3Factory
  let poolBytecode: string
  const fixture = async () => {
    const deployerFactory = await ethers.getContractFactory('MancakeV3PoolDeployer')
    const factoryFactory = await ethers.getContractFactory('PancakeV3Factory')
    const deployer_ = (await deployerFactory.deploy()) as MancakeV3PoolDeployer
    ...
    return [factory_, deployer_] as [PancakeV3Factory, MancakeV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('MancakeV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('MancakeV3Pool')
