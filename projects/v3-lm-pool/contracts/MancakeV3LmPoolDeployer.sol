...
/// versions.
contract MancakeV3LmPoolDeployer {
    struct Parameters {
        address pool;
        address masterChef;
    }
    ...
    function deploy(address pool) external onlyMasterChef returns (address lmPool) {
        ...
        lmPool = address(new MancakeV3LmPool{salt: keccak256(abi.encode(pool, masterChef, block.timestamp))}());
